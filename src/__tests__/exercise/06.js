...
test('displays the users current location', async () => {
  ...
  window.navigator.geolocation.getCurrentPosition.mockImplementation(callback => {
    promise.then(() => callback(fakePosition)) // promise.then allows us to test the loading state when the promise is still resolving (the loading spinner is on the screen)
  })
